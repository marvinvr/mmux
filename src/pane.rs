...
pub struct PaneEvents {
    ...
    /// Kitty progressive-keyboard modes pushed by the program. The active flags
    /// decide whether key modifiers need their CSI-u encoding.
    kitty_flags: Vec<u8>,
    /// Terminal replies share the pane's normal input queue, keeping the PTY writer
    /// single-owner while allowing parser callbacks to answer protocol queries.
    replies: Option<Sender<Bytes>>,
}

impl PaneEvents {
    fn new(replies: Sender<Bytes>) -> Self {
        Self {
            title: String::new(),
...
impl vt100::Callbacks for PaneEvents {
    ...
    fn unhandled_csi(
        ...
        match i1 {
            ...
            Some(b'>') => self.kitty_flags.push(param.unwrap_or(0) as u8 & 1),
            Some(b'?') => {
                if let Some(tx) = &self.replies {
                    let reply = format!("\x1b[?{}u", self.kitty_flags());
                    let _ = tx.send(Bytes::from(reply));
                }
            }
            Some(b'<') => {
...
impl Pane {
    pub fn spawn(
        ...
        // Writer thread: owns the single PTY writer and drains both user input and
        // terminal-protocol replies generated by the parser callbacks.
    ...
    /// Zero means legacy key encoding; bit 0 enables unambiguous modified keys.
    pub fn kitty_flags(&self) -> u8 {
        self.parser
            .lock()
            .ok()
            .map(|p| p.callbacks().kitty_flags())
            .unwrap_or(0)
    }
