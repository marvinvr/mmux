...
| `git.rs` | The native git panel: `GitPanel` (refresh, cursor/section nav, stage/commit/discard/stash, backgrounded pull/push), the `Overlay`/`PromptKind`/`Confirmed` modal enums (`Overlay` includes the stateless `About` card), the `DiffView` main-pane diff preview (build/follow/scroll/`diff_upkeep`) — which parses `git diff` into `DiffLine`s (kind + new-file line number + syntax-highlighted spans via [`highlight`](#the-tui-srcapp)) and, for an image file, carries a decoded `PreviewImage` shown in place of a textual diff, cached per size both as a `sixel` string (real pixels) and a `HalfCell` grid (fallback) — and the `impl App` git-action methods. **Not** a pane. |
