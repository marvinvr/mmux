...
| `git.rs` | Stateless, synchronous wrappers over the `git` CLI for the native git panel: parse `status`/`log`/`branch`; stage/discard/commit/switch/pull/push/stash; build the flattened changed-files tree. Errors come back as plain strings. |
...
| `git.rs` | The native git panel: `GitPanel` (refresh, cursor/section nav, stage/commit/discard/stash, backgrounded pull/push), the `Overlay`/`PromptKind`/`Confirmed` modal enums, and the `impl App` git-action methods. **Not** a pane. |
...
| `pane.rs` | `render_main` / `render_right` (share `render_screen`/`render_placeholder`), `main_title`, `placeholder_text`, the focused-pane cursor placement, and `paint_selection` (the drag-selection overlay). `render_right` delegates the real panel to `view::git`. |
...
| `theme.rs` | Shared styling: colors, `status_style`/`status_label`/`badge`, `header`, `project_header`, and `entry_line` (the universal sidebar row). |
