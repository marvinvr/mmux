...
| `git.rs` | The native git panel: `GitPanel` (refresh, cursor/section nav, stage/commit/discard/stash, backgrounded pull/push), the `Overlay`/`PromptKind`/`Confirmed` modal enums (`Overlay` includes the stateless `About` card), the `DiffView` main-pane diff preview (build/follow/scroll/`diff_upkeep`) — which for an image file carries a decoded `PreviewImage` shown in place of a textual diff, cached per size both as a `sixel` string (real pixels) and a `HalfCell` grid (fallback) — and the `impl App` git-action methods. **Not** a pane. |
...
| `pane.rs` | `render_main` / `render_right` (share `render_screen`/`render_placeholder`), `main_title`, `placeholder_text`, the diff preview (`render_diff`/`diff_title`; for an image it stashes a sixel for `emit_sixel`, or paints the `render_image` half-block fallback), the focused-pane cursor placement, and `paint_selection` (the drag-selection overlay). `render_right` delegates the real panel to `view::git`. |
