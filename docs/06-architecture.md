...
- **Restore.** `App::new` always calls `restore_sessions` on startup; it's a no-op when there's no
  ...
  - **Claude / Codex / Pi / Grok agents resume their conversation.** `agent.rs` is a hardcoded,
    no-config adapter detected purely by command basename. Claude and Grok let mmux *own* the id
    (launch with `--session-id <uuid>`, reattach with `--resume <uuid>`); Pi does too, but uses
    `--session-id <uuid>` for both creation and restore. Several instances in one directory
    therefore each resume their **own** thread — the id is authoritative and is never reassigned
...
- **Paste:** mmux enables bracketed paste on the outer terminal, so a paste arrives as one
  `Event::Paste` with the brackets stripped. `Pane::paste_input` re-encodes it for the program the
  way a real terminal would: line endings become `\r`, and the text is re-wrapped in
  `ESC[200~ … ESC[201~` only if the program itself enabled DECSET 2004 (read off the vt100 screen).
  Forwarding it raw made nano/vim auto-indent every line and shells/agents submit line by line.
